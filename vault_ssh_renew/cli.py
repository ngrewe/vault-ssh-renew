...
def renew():
    try:
        ...
        sys.exit(1)
    try:
        status = HostCertificate.get(
            config.ssh_host_key_path,
            config.ssh_host_cert_path
        ).read().check_renewal_required(timedelta(days=config.renewal_threshold_days))
    except RenewError:
        click.echo(click.style("An error occurred when checking certificate status", fg="red"), err=True)
        if config.debug:
            traceback.print_exc()
    ...
    try:
        ...
        click.echo(click.style("An error occurred when renewing the certificate", fg='red'), err=True)
        if config.debug:
            traceback.print_exc()
        if config.on_failure_hook:
            os.system(config.on_failure_hook)
