...
class VaultRenewer(VaultRenewInit, VaultRenewDone):
    ...
    def __init__(self, addr: ParseResult, token: str, sign_path: str, public_key: str, hostname: str, cert_path: Path):
    ...
    def renew(self) -> VaultRenewDone:
        response = requests.post(self._url, json=self._get_payload(), headers={'X-Vault-Token': self._token})
    ...
    @classmethod
    def build(cls, addr: ParseResult, token: str, sign_path: str, public_key: str, hostname: str, cert_path: Path) -> VaultRenewInit:
